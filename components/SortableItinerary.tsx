...
function SortableItem({ item }: { item: Location }) {
  ...
  return (
    <div
      ...
      className="p-4 border rounded-md flex justify-between items-center hover:shadow transition-shadow cursor-grab"
    >
      <div className="font-medium text-gray-800">
        <h4>{item.locationTitle}</h4>
        <p className="text-sm text-gray-500 trunucate max-w-xs">{`Latitude: ${item.lat}, Longitude: ${item.lng}`}</p>
      </div>
